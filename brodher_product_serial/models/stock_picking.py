...
class StockPicking(models.Model):
    ...
    def button_validate(self):
        """
        Override validate - Check stock availability and SN completion
        """
        import logging
        _logger = logging.getLogger(__name__)
        ...
        for picking in self:
            ...
            if picking.picking_type_code in ['outgoing', 'internal']:
                _logger.info(f'[VALIDATE] Checking stock availability for {picking.name}')
                ...
                if non_sn_moves:
                    # ==========================================
                    # OUTGOING: Harus klik "Check Availability" dulu
                    # ==========================================
                    if picking.picking_type_code == 'outgoing':
                        # Check apakah sudah di-check availability
                        ...
                        if unchecked_moves:
                            ...
                            _logger.warning(f'[VALIDATE] Stock belum di-check untuk: {product_names}')
                            
                            raise UserError(_(
                                '⚠️ Stock Belum Di-Check!\n\n'
                                'Produk: %s\n\n'
                                '📋 Silakan klik tombol "Check Availability" terlebih dahulu '
                                'untuk mengecek dan mereserve stock.\n\n'
                                'Setelah itu baru bisa validate transfer ini.'
                            ) % product_names)
                    
                    # ==========================================
                    # OUTGOING + INTERNAL: Cek apakah stock cukup
                    # ==========================================
                    insufficient_moves = non_sn_moves.filtered(
                        lambda m: m.state in ['confirmed', 'waiting', 'partially_available'] or
                                (m.reserved_availability < m.product_uom_qty and m.state != 'done')
                    )
                    
                    if insufficient_moves:
                        error_lines = []
                        for move in insufficient_moves[:5]:
                            # Get actual stock in source location
                            stock_qty = move.product_id.with_context(
                                location=picking.location_id.id
                            ).qty_available
                            
                            available = move.reserved_availability if move.reserved_availability else stock_qty
                            ...
                            error_lines.append(
                                ...
                                f"  Tersedia di {picking.location_id.display_name}: {stock_qty} {unit}\n"
                                f"  Kurang: {max(0, needed - stock_qty)} {unit}"
                            )
                        ...
                        # Different message for OUTGOING vs INTERNAL
                        if picking.picking_type_code == 'outgoing':
                            title = '❌ Stock Tidak Cukup untuk Delivery!'
                            actions = (
                                '📋 Pilihan:\n'
                                '1. Tunggu stock tambahan masuk\n'
                                '2. Kurangi quantity sesuai stock tersedia\n'
                                '3. Buat partial delivery (sisanya jadi backorder)'
                            )
                        else:  # internal
                            title = '❌ Stock Tidak Cukup untuk Internal Transfer!'
                            actions = (
                                '📋 Pilihan:\n'
                                '1. Pindahkan stock dari lokasi lain terlebih dahulu\n'
                        ...
                        raise UserError(_(
                            ...
                            '%s\n\n'
                            '%s'
                        ) % (title, '\n\n'.join(error_lines), actions))
            
            # ==========================================
            # CHECK 2: Serial Number Completion
            # ==========================================
            has_sn_products = any(
                move.product_id.tracking == 'serial' and 
                move.product_id.product_tmpl_id.sn_product_type
            ...
            if has_sn_products:
                ...
                if not is_complete:
                    _logger.warning(f'[VALIDATE] SN scan belum lengkap')
