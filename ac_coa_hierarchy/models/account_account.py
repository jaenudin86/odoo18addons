...
class AccountAccount(models.Model):
    ...
    def get_hierarchy_data(self):
        ...
        Called from OWL component via orm.call.
        """
        accounts = self.search([], order='code')

        # Get account_type selection labels
        acc_type_selection = dict(
            self._fields['account_type'].selection
        ) if hasattr(self._fields['account_type'], 'selection') else {}
        ...
        for acc in accounts:
            account_map[acc.id] = {
                ...
                'account_type_label': acc_type_selection.get(
                    acc.account_type, acc.account_type or ''
                ),
                ...
                'current_balance': acc.current_balance or 0.0,
            }
